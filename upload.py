    ...
    async def send_command(command):
        ...
        while len(c) > 0:
            await client.write_gatt_char(
                UUID_NORDIC_TX, bytearray(c[0:buffer_size]), True
            )
            c = c[buffer_size:]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--exec", help="script to run (load) after each upload")
    parser.add_argument(
        "--buffer_size", type=int, help="buffer size (default 20)", default=20
    )
    parser.add_argument(
        "-v",
        "--verbose",
        action="store_true",
        help="verbosity: -v show response from bangle, -vv show all DEBUG logs",
    )
